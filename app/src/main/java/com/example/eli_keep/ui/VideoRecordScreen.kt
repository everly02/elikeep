...
@Composable
fun VideoRecordScreen(existingVideoPath: String? = null, onSave: (String) -> Unit, onCancel: () -> Unit) {
    ...
    Column(modifier = Modifier
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            Text("Video Recorder", fontSize = 20.sp, fontWeight = FontWeight.Bold)
            Spacer(modifier = Modifier.height(8.dp))
            if (existingVideoPath != null) {
                Text("Existing Video: $existingVideoPath")
                Spacer(modifier = Modifier.height(8.dp))
            }
            if (isRecording) {
